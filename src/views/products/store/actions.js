...
export const addProducts = ({ commit }, add) => {
  var token = localStorage.getItem("token");
  const headers = { Authorization: `Bearer ${token}` };
  Axios.post("http://localhost:8081/products/", add, { headers }).then(
    (resp) => {
...
export const updateProducts = ({ commit }, add) => {
  var token = localStorage.getItem("token");
  const headers = { Authorization: `Bearer ${token}` };
  Axios.put("http://localhost:8081/products/" + add.id, add, { headers }).then(
    (resp) => {
...
export const deleteProducts = ({ commit }, id) => {
  var token = localStorage.getItem("token");
  const headers = { Authorization: `Bearer ${token}` };
  Axios.delete("http://localhost:8081/products/" + id, { headers }).then(
    (resp) => {
